...
namespace MyEngine {
    ...
    CSpriteFont::CSpriteFont(const char* font, int size, char cs, char ce) {
        // Initialize SDL_ttf
        if (!TTF_WasInit()) {
            TTF_Init();
        }
    ...
    std::vector<int>* CSpriteFont::CreateRows(glm::ivec4* rects, int rectsLength, int r, int padding, int& w) {
        // Blank initialize
    ...
    glm::vec2 CSpriteFont::Measure(const char* s) {
    ...
    void CSpriteFont::Draw(CSpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling, 
